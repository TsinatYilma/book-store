...
export default function ReviewFilter({ filterVisible }: { filterVisible: boolean }) {
    const [StatusDropdown, setStatusDropdown] = useState(false);
    const [SubmissionDateDropdown, setSubmissionDateDropdown] = useState(false);
    function handleDropdown(type:string){
        switch(type){
            case 'status':
                ...
                break;
            case 'submission':
                setSubmissionDateDropdown(!SubmissionDateDropdown);
                break;
        }
    }
        ...
        <div className={` absolute top-[120%] -left-1/2  w-[200px]  flex-col bg-black border-[0.15px] border-gray-700 z-10 p-3 gap-2 rounded shadow-lg ${filterVisible ? 'flex' : 'hidden'}`}>
                    <div className="relative flex flex-col justify-between  border-[0.25] border-gray-400 rounded">
                        ...
                        <div className={`StatusDropdown  top-[120%] left-0 right-0 peer-focus:flex flex-col bg-black z-10 p-3 gap-2 rounded shadow-lg ${StatusDropdown ? 'flex' : 'hidden'}`}>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <label htmlFor={'approved'} className="w-3 h-3 relative  cursor-pointer">
                                          <input type="radio" id='approved'  name="status"  className="hidden peer" />
                                          <span className="absolute top-1/2 -translate-y-1/2  w-full h-full  border-[0.15px] border-white peer-checked:bg-cyan-500 "   />
                                        </label>
                                        <p className="text-sm">Approved</p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <label htmlFor={'rejected'} className="w-3 h-3 relative  cursor-pointer">
                                          <input type="radio" id='rejected'  name="status"  className="hidden peer" />
                                          <span className="absolute top-1/2 -translate-y-1/2  w-full h-full  border-[0.15px] border-white peer-checked:bg-cyan-500 "   />
                                        </label>
                                        <p className="text-sm">Rejected</p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <label htmlFor={'pending'} className="w-3 h-3 relative  cursor-pointer">
                                          <input type="radio" id='pending'  name="status"  className="hidden peer" />
                                          <span className="absolute top-1/2 -translate-y-1/2  w-full h-full  border-[0.15px] border-white peer-checked:bg-cyan-500 "   />
                                        </label>
                    ...
                    <div className="relative flex flex-col justify-between  border-[0.25] border-gray-400 rounded">
                        <div className="flex justify-between relative w-full">
                            <h1 className="text-gray-200 text-sm p-1">Submission Date</h1>
                            <ChevronDownIcon  className={`absolute right-3 top-1/2 z-20  h-[18px] w-[18px] -translate-y-1/2 text-gray-500 ${SubmissionDateDropdown ? 'rotate-180' : ''}`} onClick={() => handleDropdown('submission')} />
                        </div>
                        <div className={`GenreDropdown  top-[120%] left-0 right-0 peer-focus:flex flex-col bg-black py-3 px-1 z-10 gap-2 rounded shadow-lg ${SubmissionDateDropdown ? 'flex' : 'hidden'}`}>
