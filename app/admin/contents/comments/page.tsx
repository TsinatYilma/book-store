...
function TableRowWithDescription() {
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      if (
        !target.closest(".description-box") &&
    ...
    <>
      <tr className="text-sm ">
        ...
        <td className="px-4 py-2 text-center text-sm relative">
          <div
            className={`flex flex-col bg-black z-10 border-[0.25px] rounded-[8px] border-gray-500  ${
              showDescription ? "absolute" : ""
            }`}
          >
            <div
              className={`relative px-4 py-1 flex items-center gap-2  justify-between   `}
            >
              <button
                className={`  text-center toggle-button ${
                  showDescription ? "text-green-400 toggle-button" : ""
                }`}
              >
                comment
              </button>
              <ChevronRightIcon
                className={`h-[15px]  w-[15px] text-white ${
                  showDescription ? "toggle-button rotate-90" : ""
                }`}
            ...
            {showDescription && (
              <div className=" py-4 shadow-md  w-full">
                <p className="text-sm text-left px-3 font-gantari text-gray-400">
                  Detail: We can keep track of all the books we’ve read, see the
                  one we are currently reading, post meet up info and there are
                  discussion questions in the app! Love it! 
                </p>
              </div>
            )}
          </div>
