...
export default async function RecentlyPublishedBooks() {
    ...
    return (
        <div className={gabarito.className +"flex flex-col w-full  mt-14 m-5 p-2 "}>
                    ...
                    books.map((book: Book) => (
                        <div key={book.name} className=" shadow-md shrink-0 w-[120px] h-[248px] sm:w-[160px] sm:h-[300px] md:w-[220px] md:h-[400px] ">
                            ...
                            <div  className="absolute inset-0 bg-black/60 flex  items-center justify-center flex-col gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <Link href='./' className='rounded-lg'><button className='fancyBorder rounded-lg bg-black text-[12px] text-md p-0.5 pl-2 pr-2 sm:p-2 sm:pl-10 sm:pr-10'>View Detail</button></Link>
                            </div>
                         </div>
